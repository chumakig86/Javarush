...
public class Solution {
    ...
    public void recursion(final String expression, int countOperation) {
        ...
        if (p3 != -1) {
            ...
            recursion(value.substring(0, p3), Integer.parseInt(df.format(currentCountOperation)));
            ...
            outputStream.reset();
            recursion(value.substring(p3 + 1), Integer.parseInt(df.format(currentCountOperation)));
        ...
        if (s.length > 5 && Character.isAlphabetic(s[0]) && s[3] == '(' && s[s.length - 1] == ')') {
            ...
            if ("sin".equals(funcName)) {
                customPrint(df, Double.parseDouble(df.format(Math.sin(Math.toRadians(Double.parseDouble(part[0]))))), countOperation, currentCountOperation);
                return;
            }
